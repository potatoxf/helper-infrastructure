package potatoxf.helper.api.text.arithmetic;

import potatoxf.helper.api.text.*;

import java.io.Reader;
import java.util.*;

/**
 * Turns a stream of characters ({@link Reader} into a stream of {@link Token}, supporting
 * lookahead.
 *
 * <p>Reads from the given input and parses it into a stream of tokens. By default, all token types
 * defined by {@link Token} are supported. Most of the features can be further tweaked by changing
 * the default settings.
 *
 * <p>By default the tokenizer operates as follows:
 *
 * <ul>
 *   <li>Consume and ignore any whitespace characters (see {@link TextChar#isWhitespace()}
 *   <li>If the current character starts a line comment, read until the end of the line and ignore
 *       all characters consumed.
 *   <li>If the current character starts a block comment, read until and end of block comment is
 *       detected.
 *   <li>If the current character is a digit, createCache a INTEGER, if a decimal separator is
 *       found, switch over to a DECIMAL (see {@link TextChar#isDigit()}. Also, if the current
 *       character is a '-', and the next is a digit, we try to read a number.
 *   <li>If the current character is a letter, createCache an ID (see {@link TextChar#isLetter()}.
 *       Once this is complete, check if the ID matches one of the supplied keywords, and convert if
 *       necessary.
 *   <li>If the current character is an opening or a closing bracket, a SYMBOL for that single
 *       character is returned
 *   <li>If the current character is one of the special id starters, all valid ID characters ({@link
 *       #isIdentifierChar(TextChar)} are consumed and returned as SPECIAL_ID
 *   <li>All other characters, especially all operators, will be read and returned as one SYMBOL.
 *       Therefore, <tt>#++*</tt> will be returned as a single symbol.
 * </ul>
 *
 * @author potatoxf
 * @date 2021/09/29
 */
class Tokenizer extends TextLookahead<Token> {
  /*
   * All supported brackets. For obvious reasons, several brackets like (( are treated as two symbols, rather than
   * operators like ** which will make one symbol
   */
   ...
   */
  protected final TextLookaheadReader input;
  /*
   ...
   *
   * @param input the input to createCache. The reader will be buffered by the implementation so
   *     that it can be effectively read character b character.
   */
  public Tokenizer(Reader input) {
    super('\0');
    this.input = new TextLookaheadReader(input, '\0');
  ...
  protected Token fetch() {
    ...
    // Treat brackets as special symbols: (( will make two consecutive symbols but ** will make a
    // single
    // symbol "**".
   ...
   *
   * <p>By default this is one of the given <tt>specialIdStarters</tt>.
   *
   * @return <tt>true</tt> if the current input is the start of a special id, <tt>false</tt>
   *     otherwise
   */
   ...
   *
   * <p>By default this is either indicated by a digit or by '-' followed by a digit or a '.'
   * followed by a digit.
   *
   * @return <tt>true</tt> if the current input is the start of a numeric constant, <tt>false</tt>
   *     otherwise
   */
   ...
   *
   * <p>By default all supplied <tt>brackets</tt> are checked. If <tt>treatSinglePipeAsBracket</tt>
   * is true, a single '|' is also treated as bracket.
   *
   * @param inSymbol determines if we're already parsing a symbol or just trying to decide what the
   *     next token is
   ...
   * @return <tt>true</tt> if the next characters of the input match the given string,
   ...
   *
   * <p>If a line comment is detected, any characters indicating this are consumed by this method if
   * <tt>consume</tt> is <tt>true</tt>.
   *
   * @param consume determines if the matched comment start should be consumed immediately
   * @return <tt>true</tt> if the next character(s) of the input start a line comment,
   *     <tt>false</tt> otherwise
   */
   ...
   *
   * <p>If a block comment is detected, any characters indicating this are consumed by this method
   * if <tt>consume</tt> is <tt>true</tt> .
   *
   * @param consume determines if the block comment starter is to be consumed if found or not
   * @return <tt>true</tt> if the next character(s) of the input start a block comment,
   *     <tt>false</tt> otherwise
   */
   ...
   *
   * <p>If an end of block comment is detected, any characters indicating this are consumed by this
   * method
   *
   * @return <tt>true</tt> if the next character(s) of the input end a block comment, <tt>false</tt>
   *     otherwise
   */
  ...
  protected void skipBlockComment() {
    ...
    problemCollector.add(TextParseError.error(input.current(), "Premature end of block comment"));
  }
  ...
  protected Token fetchString() {
    ...
    while (!input.current().isNewLine()
        && !input.current().is(separator)
        && !input.current().isEndOfInput()) {
        ...
        if (!handleStringEscape(separator, escapeChar, result)) {
          problemCollector.add(
              TextParseError.error(
                  input.next(),
                  String.format(
                      "Cannot use '%s' as escaped character", input.next().getStringValue())));
        }
      } else {
    ...
    if (input.current().is(separator)) {
      ...
      problemCollector.add(
          TextParseError.error(input.current(), "Premature end of string constant"));
    }
    return result;
  }
   ...
   *
   * <p>The escape character is already consumed. Therefore the input points at the character to
   * escape. This method must consume all escaped characters.
   *
   ...
   *
   * <p>By default, only letters can start identifiers
   *
   * @return <tt>true</tt> if the underlying input is looking at a valid identifier starter,
   *     <tt>false</tt> otherwise
   */
  ...
  protected Token fetchId() {
    ...
    if (!input.current().isEndOfInput()
        && specialIdTerminators.contains(input.current().getValue())) {
  ...
  /**
   * Determines if the given TextChar is a valid identifier part.
   *
   * <p>By default, letters, digits and '_' are valid identifier parts.
   *
   * @param current the character to check
   * @return <tt>true</tt> if the given TextChar is a valid identifier part, <tt>false</tt>
   *     otherwise
   */
  protected boolean isIdentifierChar(TextChar current) {
   ...
   *
   * <p>A symbol are one or two characters, which don't match any other token type. In most cases,
   * this will be operators like + or *.
   *
  ...
  /**
   * Determines if the given TextChar is a symbol character.
   *
   * <p>By default these are all non-control characters, which don't match any other class (letter,
   * digit, whitepsace)
   *
   * @param ch the character to check
   * @return <tt>true</tt> if the given character is a valid symbol character, <tt>false</tt>
   *     otherwise
   */
  @SuppressWarnings("squid:S1067")
  protected boolean isSymbolCharacter(TextChar ch) {
    ...
    while (input.current().isDigit()
      ...
      if (input.current().is(groupingSeparator)) {
        ...
        if (result.is(Token.TokenType.DECIMAL) || result.is(Token.TokenType.SCIENTIFIC_DECIMAL)) {
          problemCollector.add(
              TextParseError.error(input.current(), "Unexpected decimal separators"));
        } else {
        ...
        if (result.is(Token.TokenType.SCIENTIFIC_DECIMAL)) {
          problemCollector.add(
              TextParseError.error(input.current(), "Unexpected scientific notation separators"));
        } else {
   ...
   * Determines if keywords are case sensitive.
   *
   * <p>By default, keywords aren't case sensitive. Therefore True and true are the same keyword.
   *
   ...
   *
   * <p>This must be setup before any call to {@link #addKeyword(String)} as this will determine
   * internal data structures
   *
   * @param keywordsCaseSensitive <tt>true</tt> if keywords should be treated as case sensitive,
   *     <tt>false</tt> otherwise (default)
   */
   ...
   *
   * <p>Detection will be case insensitive. Only ID tokens (identifiers) are checked against the
   * given keywords, therefore a keyword must be a valid identifier.
   *
   * @param keyword the keyword to be added to the list of known keywords.
   */
  public void addKeyword(String keyword) {
    keywords.put(
        keywordsCaseSensitive ? keyword.intern() : keyword.toLowerCase().intern(), keyword);
  }

  /**
   * Adds character as a special id starter. The given character must be followed by a valid id to
   * be recognized as SPECIAL_ID.
   *
  ...
  /**
   * Adds character as a special id terminator. The given character can be placed at the end of an
   * id to be recognized as SPECIAL_ID.
   *
   ...
   *
   * <p>By default " and ' are registered as string delimiters, where string enclosed by " can have
   ...
   * @param escapeCharacter the character used to start an escape sequence or \0 to indicate that
   ...
   *
   * <p>The default separator used is '.'
   *
   * @return the character which is detected as separator between the integer and the decimal part
   *     of a number
   */
   ...
   *
   * <p>The default separator used is '.'. When adapting this for language dependent inputs (e.g.
   * using ',' as decimal separator) this value should probably not be changed, as it is used in the
   * output (content) of the Tokens and has no effect what kind of numbers are being accepted.
   *
  ...
  /**
   * Sets the decimal separator used in the content of DECIMAL tokens. This can differ from the
   * character set via {@link #setDecimalSeparator(char)} which is used to recognize decimal
   * numbers. Therefore language dependent input can be parsed with a constant output being language
   * independent.
   *
   * @param effectiveDecimalSeparator the character used as decimal separator in the content of
   *     decimal tokens
   */
  ...
  /**
   * Returns the grouping separator which can be used in numbers for group digits (e.g. in
   * thousands).
   *
   * <p>This character will be accepted in numbers, but ignored (not added to the content). The
   * default value is '_'.
   *
  ...
  public String toString() {
    // We check the internal buffer first to that no further parsing is triggered by calling
    // toString()
   ...
   *
   * @return <tt>true</tt> if the current token is not an "end of input" token, <tt>false</tt>
   *     otherwise.
   */
  public boolean more() {
   ...
   *
   * @return <tt>true</tt> if the current token is an "end of input" token, <tt>false</tt>
   *     otherwise.
   */
  ...
  /**
   * Adds a createCache error to the internal problem collector.
   *
   * <p>It is preferred to collect as much errors as possible and then fail with an exception
   * instead of failing at the first problem. Often syntax errors can be worked out by the parser
   * and we can report a set of errors at apply.
   *
   * @param pos the position of the error. Note that {@link Token} implements {@link TextPosition}.
   *     Therefore the current token is often a good choice for this parameter.
   * @param message the message to describe the error. Can contain formatting parameters like %s or
   *     %d as defined by {@link String#format(String, Object...)}
   * @param parameters Contains the parameters used to format the given message
   */
  public void addError(TextPosition pos, String message, Object... parameters) {
    getProblemCollector().add(TextParseError.error(pos, String.format(message, parameters)));
  }

  /**
   * Adds a warning to the internal problem collector.
   *
   * <p>A warning indicates an anomaly which might lead to an error but still, the parser can
   * continue to complete its work.
   *
   * @param pos the position of the warning. Note that {@link Token} implements {@link
   *     TextPosition}. Therefore the current token is often a good choice for this parameter.
   * @param message the message to describe the earning. Can contain formatting parameters like %s
   *     or %d as defined by {@link String#format(String, Object...)}
   * @param parameters Contains the parameters used to format the given message
   */
  public void addWarning(TextPosition pos, String message, Object... parameters) {
    getProblemCollector().add(TextParseError.warning(pos, String.format(message, parameters)));
  }
  ...
  /**
   * Throws a {@link TextParseException} if an error or warning occurred while parsing the input
   *
   * @throws TextParseException if an error or warning occurred while parsing.
   */
  public void throwOnErrorOrWarning() throws TextParseException {
    if (!getProblemCollector().isEmpty()) {
      throw TextParseException.create(getProblemCollector());
    }
  }

  /**
   * Throws a {@link TextParseException} if an error occurred while parsing the input.
   *
   * <p>All warnings which occurred will be ignored.
   *
   * @throws TextParseException if an error occurred while parsing.
   */
  public void throwOnError() throws TextParseException {
    for (TextParseError e : getProblemCollector()) {
      if (e.getTextParseSeverity() == TextParseError.Severity.ERROR) {
        throw TextParseException.create(getProblemCollector());
      }
    }
  }
