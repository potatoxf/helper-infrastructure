...
import potatoxf.helper.api.text.TextChar;
import potatoxf.helper.api.text.TextPosition;

/**
 * Represents a token of text read from a {@link Tokenizer}.
 *
 * <p>A token consists of a position, a type and up to three string values. The first is the
 * <tt>trigger</tt>. Along with the type this uniquely identifies what kind of token we're looking
 * at. For example the input <tt>"Hello"</tt>, the id <tt>Hello</tt> and a special id like
 * <tt>#Hello</tt> all have the same content (Hello). However, they will have different types
 * (<tt>STRING</tt>, <tt>ID</tt>, <tt>SPECIAL_ID</tt>). However, the two special ids <tt>$Hello</tt>
 * and <tt>#Hello</tt> will both have the same token type and content. Those will differ in their
 * <tt>trigger</tt> which will be <tt>#</tt> and <tt>$</tt> respectively.
 *
 * <p>As already shown, the <tt>content</tt> contains the effective content of the token which
 * should be used for further processing. Finally the <tt>source</tt> contains the complete text
 * which was consumed while reading the token. If we look at a string constant <tt>"Hello"</tt> the
 * content will be <tt>Hello</tt> and the source will be <tt>"Hello"</tt>.
 *
 * <p>The basic token types supported are:
 *
 * <ul>
 *   <li>ID: Represents an name or reference like a function name or variable.
 *   <li>SPECIAL_ID: Represents an id which starts with a special character like $ or # and
 *       continues with an id
 *   <li>STRING: Represents a string constant
 *   <li>DECIMAL: Represents a decimal constant
 *   <li>SCIENTIFIC_DECIMAL: Represents a decimal constant with a scientific notation
 *   <li>INTEGER: Represents an integer constant
 *   <li>SYMBOL: Represents any combination of "special" chars like + - ** etc. This will also be
 *       all bracket
 *   <li>KEYWORD: Represents an ID which was identified as a keyword
 *   <li>EOI: Signals the end of input
 * </ul>
 *
 * @author potatoxf
 * @date 2021/09/29
 */
class Token extends TextPosition {
   ...
   */
  public static Token create(TokenType type, TextPosition pos) {
    Token result = new Token();
    result.type = type;
    result.line = pos.atLine();
    result.position = pos.atPosition();

    return result;
  }

  /**
   * Creates a new token with the given type, using the TextChar a initial trigger and content.
   *
   * @param type the type if this token. The supplied TextChar will be used as initial part of the
   *     trigger to further specify the token
   * @param ch first character of the content and trigger of this token. Also specifies the position
   *     of the token.
   * @return a new token which is initialized with the given TextChar
   */
  public static Token createAndFill(TokenType type, TextChar ch) {
    Token result = new Token();
    result.type = type;
    result.line = ch.atLine();
    result.position = ch.atPosition();
  ...
  /**
   * Adds the given TextChar to the trigger (and the source) but not to the content
   *
   * @param ch the character to add to the trigger and source
   * @return <tt>this</tt> to support fluent method calls
   */
  public Token addToTrigger(TextChar ch) {
    trigger += ch.getValue();
    internTrigger = null;
    source += ch.getValue();
    return this;
  ...
  /**
   * Adds the given TextChar to the source of this token, but neither to the trigger nor to the
   * content.
   *
   * @param ch the character to add to the source
   * @return <tt>this</tt> to support fluent method calls
   */
  public Token addToSource(TextChar ch) {
    source += ch.getValue();
    return this;
  }

  /**
   * Adds the given TextChar to the content (and the source) but not to the trigger
   *
   * @param ch the character to add to the content and source
   * @return <tt>this</tt> to support fluent method calls
   */
  public Token addToContent(TextChar ch) {
   ...
   *
   * <p>This will neither change the content nor the source of this token.
   *
   ...
   *
   * <p>This will neither change the trigger nor the content of this token.
   *
   ...
   *
   * <p>This will neither change the trigger nor the source of this token.
   *
   ...
   *
   * @return <tt>true</tt> if this is an end of input token (with EOI as type), <tt>false</tt>
   *     otherwise
   */
   ...
   *
   * @return <tt>false</tt> if this is an end of input token (with EOI as type), <tt>true</tt>
   *     otherwise
   */
   ...
   * @return <tt>true</tt> if this token matches the given type and trigger, <tt>false</tt>
   *     otherwise
   */
  ...
  public boolean matches(TokenType type, String trigger) {
    ...
    return Whether.equals(trigger.intern(), getTrigger());
  }
   ...
   * @return <tt>true</tt> if this token was triggered by one of the given triggers, <tt>false</tt>
   *     otherwise
   */
  public boolean wasTriggeredBy(String... triggers) {
    for (String trigger : triggers) {
      if (trigger != null && Whether.equals(trigger, getTrigger())) {
        return true;
      }
    }
    return false;
   ...
   *
   * <p>If a list of <tt>symbols</tt> is given, this method checks that the trigger matches one of
   * them.
   *
   * @param symbols the symbols to check for. If the list es empty, only the token type is checked.
   * @return <tt>true</tt> if this token is a symbol and matches one of the given <tt>symbols</tt>
   ...
   * them.
   *
   * @param keywords the keywords to check for. If the list es empty, only the token type is
   *     checked.
   * @return <tt>true</tt> if this token is a keyword and matches one of the given <tt>keywords</tt>
   *     if the list is not empty.
   */
   ...
   *
   * <p>If a list of <tt>values</tt> is given, this method checks that the content matches one of
   * them.
   *
   * @param values the values to check for. If the list es empty, only the token type is checked.
   * @return <tt>true</tt> if this token is an identifier and matches one of the given
   *     <tt>values</tt> if the list is not empty.
   */
   ...
   *
   * <p>If a list of <tt>triggers</tt> is given, this method checks that the trigger matches one of
   * them.
   *
   * @param triggers the triggers to check for. If the list es empty, only the token type is
   *     checked.
   * @return <tt>true</tt> if this token is a special identifier and matches one of the given
   *     <tt>triggers</tt> if the list is not empty.
   */
   ...
   *
   * <p>If a list of <tt>contents</tt> is given, this method checks that the content matches one of
   * them.
   *
   * @param trigger the trigger of the special id
   * @param contents the content to check for. If the list es empty, only the token type and the
   *     trigger is checked.
   * @return <tt>true</tt> if this token is a special identifier with the given trigger. If
   *     <tt>contents</tt> is not empty, the content must also match one of the elements.
   */
  ...
  public String toString() {
    return getType().toString() + ":" + getSource() + " (" + line + ":" + position + ")";
  }
