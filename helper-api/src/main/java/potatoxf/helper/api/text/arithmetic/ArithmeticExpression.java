...
import potatoxf.helper.api.text.TextParseError;
import potatoxf.helper.api.text.TextParseException;
...
public final class ArithmeticExpression {
  ...
  public static Parser create(String input) {
    return new Parser(new StringReader(input), ArithmeticFunction.DEFAULT_FUNCTION_TABLE);
  }

  public static Parser create(String input, Map<String, ArithmeticFunction> functionTable) {
    return new Parser(new StringReader(input), functionTable);
  }

  public static Parser create(Reader input) {
    return new Parser(input, ArithmeticFunction.DEFAULT_FUNCTION_TABLE);
  }

  public static Parser create(Reader input, Map<String, ArithmeticFunction> functionTable) {
  ...
  private ArithmeticData checkArg(ArithmeticData arithmeticData) {
    ArithmeticData wrapper =
        arithmeticData != null ? new ArithmeticData(arithmeticData) : new ArithmeticData();
  ...
  public static final class Parser {
    ...
    /** 函数表 */
    private final Map<String, ArithmeticFunction> functionTable;
    /** 变量名列表 */
    ...
    private Parser(Reader input, Map<String, ArithmeticFunction> functionTable) {
      this.tokenizer = new Tokenizer(input);
      this.tokenizer.setProblemCollector(errors);
      this.functionTable = Tools.safeValue(functionTable);
    }
    ...
    private Expression expression() {
      ...
      if (tokenizer.current().isSymbol("&&")) {
        ...
        return reOrder(left, right, ExpressionBinary.Op.AND);
      }
      if (tokenizer.current().isSymbol("||")) {
        tokenizer.consume();
        Expression right = expression();
        return reOrder(left, right, ExpressionBinary.Op.OR);
      }
      return left;
    }
    ...
    private Expression relationalExpression() {
      ...
      if (tokenizer.current().isSymbol("<")) {
        ...
        return reOrder(left, right, ExpressionBinary.Op.LT);
      }
      if (tokenizer.current().isSymbol("<=")) {
        tokenizer.consume();
        Expression right = relationalExpression();
        return reOrder(left, right, ExpressionBinary.Op.LT_EQ);
      }
      if (tokenizer.current().isSymbol("=")) {
        tokenizer.consume();
        Expression right = relationalExpression();
        return reOrder(left, right, ExpressionBinary.Op.EQ);
      }
      if (tokenizer.current().isSymbol(">=")) {
        tokenizer.consume();
        Expression right = relationalExpression();
        return reOrder(left, right, ExpressionBinary.Op.GT_EQ);
      }
      if (tokenizer.current().isSymbol(">")) {
        tokenizer.consume();
        Expression right = relationalExpression();
        return reOrder(left, right, ExpressionBinary.Op.GT);
      }
      if (tokenizer.current().isSymbol("!=")) {
        tokenizer.consume();
        Expression right = relationalExpression();
        return reOrder(left, right, ExpressionBinary.Op.NEQ);
      }
      return left;
    }
    ...
    private Expression term() {
      ...
      if (tokenizer.current().isSymbol("+")) {
        ...
        return reOrder(left, right, ExpressionBinary.Op.ADD);
      }
      if (tokenizer.current().isSymbol("-")) {
        tokenizer.consume();
        Expression right = term();
        return reOrder(left, right, ExpressionBinary.Op.SUBTRACT);
      }
    ...
    private Expression product() {
      ...
      if (tokenizer.current().isSymbol("*")) {
        ...
        return reOrder(left, right, ExpressionBinary.Op.MULTIPLY);
      }
      if (tokenizer.current().isSymbol("/")) {
        tokenizer.consume();
        Expression right = product();
        return reOrder(left, right, ExpressionBinary.Op.DIVIDE);
      }
      if (tokenizer.current().isSymbol("%")) {
        tokenizer.consume();
        if (tokenizer.current().isEnd() || !tokenizer.current().isNumber()) {
          Expression expression =
              reOrder(left, new ExpressionConstant(0.01), ExpressionBinary.Op.MULTIPLY);
          if (tokenizer.current().isNotEnd()) {
            tokenizer.consume();
            Expression right = product();
            return reOrder(expression, right, ExpressionBinary.Op.MULTIPLY);
          }
          return expression;
        }
        Expression right = product();
        return reOrder(left, right, ExpressionBinary.Op.MODULO);
      }
      return left;
    }
     ...
     */
    private Expression reOrder(Expression left, Expression right, ExpressionBinary.Op op) {
      if (right instanceof ExpressionBinary) {
        ExpressionBinary rightOp = (ExpressionBinary) right;
        if (!rightOp.isSealed() && rightOp.getOp().getPriority() == op.getPriority()) {
          replaceLeft(rightOp, left, op);
          return right;
        }
      }
      return new ExpressionBinary(op, left, right);
    }

    private void replaceLeft(ExpressionBinary target, Expression newLeft, ExpressionBinary.Op op) {
      if (target.getLeft() instanceof ExpressionBinary) {
        ExpressionBinary leftOp = (ExpressionBinary) target.getLeft();
        if (!leftOp.isSealed() && leftOp.getOp().getPriority() == op.getPriority()) {
          replaceLeft(leftOp, newLeft, op);
          return;
        }
      }
      target.setLeft(new ExpressionBinary(op, newLeft, target.getLeft()));
    }
    ...
    private Expression power() {
      ...
      if (tokenizer.current().isSymbol("^") || tokenizer.current().isSymbol("**")) {
        ...
        return reOrder(left, right, ExpressionBinary.Op.POWER);
      }
      return left;
    }
    ...
    private Expression atom() {
      if (tokenizer.current().isSymbol("-")) {
        tokenizer.consume();
        ExpressionBinary result =
            new ExpressionBinary(ExpressionBinary.Op.SUBTRACT, new ExpressionConstant(0d), atom());
        result.seal();
        return result;
      }
      ...
      if (tokenizer.current().isSymbol("(")) {
        ...
        if (result instanceof ExpressionBinary) {
          ((ExpressionBinary) result).seal();
        }
        expect(")");
        return result;
      }
      if (tokenizer.current().isSymbol("|")) {
        tokenizer.consume();
        ExpressionFunction call = new ExpressionFunction(ArithmeticFunction.ABS);
    ...
    private Expression literalAtom() {
      ...
      return ExpressionConstant.EMPTY;
    }
    ...
    private Expression functionCall() {
      ...
      ArithmeticFunction fun = functionTable.get(funToken.getContents());
      if (fun == null) {
        errors.add(
            TextParseError.error(
      ...
      ExpressionFunction call = new ExpressionFunction(fun);
      tokenizer.consume();
