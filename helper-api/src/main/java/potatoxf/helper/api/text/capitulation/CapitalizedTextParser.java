 ...
 * @date 2021/06/19
 */
public abstract class CapitalizedTextParser {
   ...
   * @return 返回正规的金额数字
   */
  public String parse(@Nonnull BigDecimal number) {
   ...
   * @return 返回正规的金额数字
   */
  public String parse(@Nonnull BigInteger number) {
   ...
   * @return 返回正规的金额数字
   */
  public String parse(@Nonnull String number) {
    ...
    if (decimal != null) {
      ...
      bigDecimal = bigDecimal.setScale(this.decimalPlaces, RoundingMode.HALF_UP);
  ...
  public void setDecimalPlaces(int decimalPlaces) {
    if (decimalPlaces > configMaxIntegerLength()) {
      throw new IllegalArgumentException(
