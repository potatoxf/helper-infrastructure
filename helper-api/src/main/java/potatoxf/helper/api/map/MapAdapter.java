...
public class MapAdapter<K, V> implements Map<K, V>, Cloneable, Serializable {
  ...
  public MapAdapter(Map<K, V> proxyMap) {
    this.proxyMap = Objects.requireNonNull(proxyMap);
  }
   ...
   * of this call is equivalent to that of calling {@link #put(Object, Object) put(k, v)} on this
   * map apply for each mapping from key <tt>k</tt> to value <tt>v</tt> in the specified map. The
   * behavior of this operation is undefined if the specified map is modified while the operation is
   * in progress.
   *
   ...
   * this call returns.
   *
   * @throws UnsupportedOperationException if the <tt>reset</tt> operation is not supported by this
   *     map
   */
  @Override
  public void clear() {
   ...
   * <tt>Set.remove</tt>, <tt>removeAll</tt>, <tt>retainAll</tt>, and <tt>reset</tt> operations. It
   ...
   * <tt>retainAll</tt> and <tt>reset</tt> operations. It does not support the <tt>add</tt> or
   ...
   * <tt>removeAll</tt>, <tt>retainAll</tt> and <tt>reset</tt> operations. It does not support the
  ...
  public MapAdapter<K, V> clone() {
    try {
      return (MapAdapter<K, V>) super.clone();
