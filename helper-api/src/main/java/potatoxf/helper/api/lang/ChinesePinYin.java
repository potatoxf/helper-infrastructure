...
import potatoxf.helper.api.StringCase;
import potatoxf.helper.api.iterate.LineIterable;
import potatoxf.helper.api.map.GroupStatisticalMap;
...
public final class ChinesePinYin {
  ...
  static {
    LineIterable lineIterable =
        new LineIterable(
            ChinesePinYin.class.getResourceAsStream("helper/api/ChinesePinYin.info"), true);
  ...
  public static List<List<Object>> getAllPossiblePinYin(String input, String trimChar) {
    ...
    if (count < 0) {
      List<Object> data = new ArrayList<>(chars.length);
      for (char c : chars) {
      ...
      GroupStatisticalMap<Character> statisticalMap = new GroupStatisticalMap<>(chars.length, 1);
      for (char c : chars) {
        if (trimChar.indexOf(c) >= 0) {
          continue;
        }
