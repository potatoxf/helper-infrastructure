...
public class MappingFolder {
  ...
  public MappingFolder(String builtRootDirPath, String outerRootDirPath) {
    this.builtRootDirPath = Tools.safePath(false, true, builtRootDirPath);
    this.outerRootDirPath = Tools.safePath(false, true, outerRootDirPath);
  }
  ...
  public final File getOuterFile(String resourcePath) throws IOException {
    ...
    if (!file.exists()) {
      ...
      HelperOnStream.write(file, getBuiltInputStream(resourcePath));
      if (!file.exists()) {
        throw new FileNotFoundException(
  ...
  private URL getResourceUrl(String resourcePath) throws IOException {
    ...
    try {
      HelperOnCollection.addAll(list, ClassLoader.getSystemResources(builtPath));
    } catch (IOException e) {
