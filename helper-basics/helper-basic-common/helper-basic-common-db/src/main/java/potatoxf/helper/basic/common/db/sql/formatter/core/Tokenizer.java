...
public class Tokenizer {
  ...
  public Tokenizer(DialectConfig cfg) {
    ...
    this.OPERATOR_PATTERN =
        Pattern.compile(
            SqlFormatterHelper.createOperatorRegex(
    ...
    this.LINE_COMMENT_PATTERN =
        Pattern.compile(
            SqlFormatterHelper.createLineCommentRegex(new JSLikeList<>(cfg.lineCommentTypes)));

    this.RESERVED_TOP_LEVEL_PATTERN =
        Pattern.compile(
            SqlFormatterHelper.createReservedWordRegex(
                new JSLikeList<>(cfg.reservedTopLevelWords)));
    this.RESERVED_TOP_LEVEL_NO_INDENT_PATTERN =
        Pattern.compile(
            SqlFormatterHelper.createReservedWordRegex(
                new JSLikeList<>(cfg.reservedTopLevelWordsNoIndent)));
    this.RESERVED_NEWLINE_PATTERN =
        Pattern.compile(
            SqlFormatterHelper.createReservedWordRegex(new JSLikeList<>(cfg.reservedNewlineWords)));
    this.RESERVED_PLAIN_PATTERN =
        Pattern.compile(
            SqlFormatterHelper.createReservedWordRegex(new JSLikeList<>(cfg.reservedWords)));

    this.WORD_PATTERN =
        Pattern.compile(SqlFormatterHelper.createWordRegex(new JSLikeList<>(cfg.specialWordChars)));
    this.STRING_PATTERN =
        Pattern.compile(SqlFormatterHelper.createStringRegex(new JSLikeList<>(cfg.stringTypes)));

    this.OPEN_PAREN_PATTERN =
        Pattern.compile(SqlFormatterHelper.createParenRegex(new JSLikeList<>(cfg.openParens)));
    this.CLOSE_PAREN_PATTERN =
        Pattern.compile(SqlFormatterHelper.createParenRegex(new JSLikeList<>(cfg.closeParens)));

    this.INDEXED_PLACEHOLDER_PATTERN =
        SqlFormatterHelper.createPlaceholderRegexPattern(
    ...
    this.STRING_NAMED_PLACEHOLDER_PATTERN =
        SqlFormatterHelper.createPlaceholderRegexPattern(
            ...
            SqlFormatterHelper.createStringPattern(new JSLikeList<>(cfg.stringTypes)));
  }
  ...
  private Token getNextToken(String input, Token previousToken) {
    return SqlFormatterHelper.firstNotnull(
  ...
  private String getEscapedPlaceholderKey(String key, String quoteChar) {
    return key.replaceAll(Tools.escapeRegExp(key) + quoteChar, quoteChar);
  }
