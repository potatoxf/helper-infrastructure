...
import potatoxf.helper.api.function.ConsumerThrow;
...
public class SqlHelper {
  ...
  public static boolean runScript(
    ...
    try {
      if (sendFullScript) {
        // todo
        String command = ""; // IOHelper.readAllString(lineReader);
        logger.accept(command);
        executeStatement.apply(command);
      } else {
        String line;
  ...
  public static void log(String sql, List<Object> args, Consumer<String> logger) {
    ...
    if (size == 0) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        String index = HelperOnNumber.showNumber(i + 1, 2, '0');
        String argType = HelperOnString.layout(arg.getClass().getSimpleName(), max, ' ', true);
